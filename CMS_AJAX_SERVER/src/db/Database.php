...
class Database {
    ...
    static function dbConnect() {
        ...
        if (Database::$dbConnection == NULL) {
            Database::$dbConnection = mysqli_connect("localhost", "root", "", "monsterjob");
        }
    ...
    static function insert($tableName, $columnCollection) {
        ...
        $queryBuilder = substr_replace($queryBuilder, ")", strlen($queryBuilder) - 1); // броим къде ще поставим скобата, но тъй като по принцип се брои от 0, но пък дължината на стринга показва реалният брой на символите, ние пишем -1, за да може мястото на поставяне на съответства с броенето в програмирането
        $queryBuilder .= ' VALUES(';
